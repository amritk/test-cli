...
```sh
galaxy planets create --bearer-auth "$BEARER_AUTH" --name 'Mars' --type 'terrestrial'
```
...
```sh
galaxy planets update '1' --bearer-auth "$BEARER_AUTH" --name 'Mars' --type 'terrestrial'
```
...
```sh
galaxy planets upload-image '1' --bearer-auth "$BEARER_AUTH"
```
